...
trait Solver extends GameDef {
  ...
  def newNeighborsOnly(neighbors: Stream[(Block, List[Move])],
    ...
    neighbors.filter((tuple: (Block, List[Move])) => !explored(tuple._1))
   ...
   */
    def from(initial: Stream[(Block, List[Move])], explored: Set[Block]): Stream[(Block, List[Move])] = {
      val more = for {
        (block, path) <- initial
        neighbor <- newNeighborsOnly(neighborsWithHistory(block, path), explored)
      } yield neighbor
   ...
   */
  lazy val pathsToGoal: Stream[(Block, List[Move])] = pathsFromStart.filter((tuple: (Block, List[Move])) => done(tuple._1))
